...
@patch('app.webapps.core.webapp_kubernetes_service.apply_to_kubernetes')
@patch('app.shared.k8s.cluster_selection.ClusterSelectionService.get_cluster_with_least_load_or_raise')
def test_create_worker_exceeds_environment_limits_returns_400(
    mock_get_cluster, mock_apply, mock_gateway,
    client, admin_token, test_instance,
):
    """
    Create worker with CPU, memory or replicas above environment limits returns 400.
    Environment has default settings (max_cpu_cores=2, max_memory=2048, max_pods=5).
    """
    from unittest.mock import MagicMock
...
@patch('app.webapps.core.webapp_kubernetes_service.apply_to_kubernetes')
@patch('app.shared.k8s.cluster_selection.ClusterSelectionService.get_cluster_with_least_load_or_raise')
def test_create_worker_exceeds_cpu_limit_returns_400(
    mock_get_cluster, mock_apply, mock_gateway,
    client, admin_token, test_instance,
):
    """Create worker with CPU above max_cpu_cores returns 400 with message about CPU limit."""
    from unittest.mock import MagicMock
