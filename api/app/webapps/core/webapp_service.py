...
class WebappService:
    ...
    def create_webapp(self, dto: WebappCreate) -> Webapp:
        ...
        validate_url_for_exposure(
            dto.url, dto.settings.exposure.type, dto.settings.exposure.visibility.value
        )
        ...
        self._deploy_to_kubernetes(
            webapp, instance.environment_id, cluster, cluster_instance
        )
    ...
    def update_webapp(self, uuid: UUID, dto: WebappUpdate) -> Webapp:
        ...
        has_changes = (
            dto.settings is not None or dto.url is not None or dto.enabled is not None
        )

        enabled_changed = self._update_webapp_fields(webapp, dto)
        cluster_instance = get_or_create_cluster_instance(
            self.repository, self.db, webapp
        )
        ...
        if enabled_changed["changed"]:
            ...
            self._deploy_to_kubernetes(
                webapp, webapp.instance.environment_id, cluster, cluster_instance
            )
    ...
    def _validate_exposure_settings(self, settings_dict: dict, cluster: any) -> None:
        ...
        if "exposure" in settings_dict:
            if "type" in settings_dict["exposure"]:
                validate_exposure_type_for_cluster(
                    cluster, settings_dict["exposure"]["type"]
                )
            if "visibility" in settings_dict["exposure"]:
                validate_visibility_for_cluster(
                    cluster, settings_dict["exposure"]["visibility"]
                )

    def _build_webapp_entity(
        self, dto: WebappCreate, instance_id: int
    ...
    def _update_webapp_fields(
        self, webapp: ApplicationComponentModel, dto: WebappUpdate
    ) -> dict:
        ...
        exposure_visibility = final_settings.get("exposure", {}).get(
            "visibility", "cluster"
        )
    ...
    def _delete_from_kubernetes_safe(
        self, webapp: ApplicationComponentModel, cluster: any
    ) -> None:
