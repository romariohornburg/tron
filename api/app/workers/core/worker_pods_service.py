"""Kubernetes pods operations for workers. Isolated from business logic."""
...
def get_worker_pods_from_cluster(
    cluster: ClusterModel, application_name: str, component_name: str
) -> List[Dict[str, Any]]:
    """Get pods for worker from cluster."""
    ...
    pods = k8s_client.list_pods(
        namespace=application_name, label_selector=label_selector
    )
...
def delete_worker_pod_from_cluster(
    cluster: ClusterModel, application_name: str, pod_name: str
) -> None:
...
def get_worker_pod_logs_from_cluster(
...
def exec_worker_pod_command_from_cluster(
    cluster: ClusterModel,
    application_name: str,
    pod_name: str,
    command: List[str],
