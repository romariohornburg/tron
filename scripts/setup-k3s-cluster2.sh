...
KUBECONFIG_FILE="${KUBECONFIG_FILE:-./volumes/kubeconfig2/kubeconfig2.yaml}"
ENVIRONMENT_NAME="${ENVIRONMENT_NAME:-local}"
CLUSTER_NAME="${CLUSTER_NAME:-local-cluster2}"
CLUSTER_API_ADDRESS="${CLUSTER_API_ADDRESS:-https://k3s-server2:5444}"  # Port 5444 as configured in docker-compose.cluster2.yaml
...
echo -e "${GREEN}🚀 Validando/Criando token de API para cluster2...${NC}"
...
if ! command -v kubectl &> /dev/null; then
    ...
    if [ -z "$CLUSTER_API_ADDRESS" ] || [ "$CLUSTER_API_ADDRESS" = "https://127.0.0.1:6443" ]; then
        ...
        if [ -z "$CLUSTER_API_ADDRESS" ]; then
            CLUSTER_API_ADDRESS="https://127.0.0.1:5444"
        fi
