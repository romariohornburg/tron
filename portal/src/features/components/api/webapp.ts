...
export const webappComponentsApi = {
  list: async (organizationUuid: string): Promise<ApplicationComponent[]> => {
    const response = await api.get<ApplicationComponent[]>(`/organizations/${organizationUuid}/application_components/webapp/`)
    return response.data
  },
  get: async (organizationUuid: string, uuid: string): Promise<ApplicationComponent> => {
    const response = await api.get<ApplicationComponent>(`/organizations/${organizationUuid}/application_components/webapp/${uuid}`)
    return response.data
  },
  create: async (organizationUuid: string, data: ApplicationComponentCreate): Promise<ApplicationComponent> => {
    ...
    const response = await api.post<ApplicationComponent>(`/organizations/${organizationUuid}/application_components/webapp/`, data)
    return response.data
  },
  update: async (organizationUuid: string, uuid: string, data: Partial<ApplicationComponentCreate>): Promise<ApplicationComponent> => {
    const response = await api.put<ApplicationComponent>(`/organizations/${organizationUuid}/application_components/webapp/${uuid}`, data)
    return response.data
  },
  delete: async (organizationUuid: string, uuid: string): Promise<void> => {
    await api.delete(`/organizations/${organizationUuid}/application_components/webapp/${uuid}`)
  },
  getPods: async (organizationUuid: string, uuid: string): Promise<Pod[]> => {
    const response = await api.get<Pod[]>(`/organizations/${organizationUuid}/application_components/webapp/${uuid}/pods`)
    return response.data
  },
  deletePod: async (organizationUuid: string, uuid: string, podName: string): Promise<void> => {
    await api.delete(`/organizations/${organizationUuid}/application_components/webapp/${uuid}/pods/${podName}`)
  },
  ...
  execPodCommand: async (organizationUuid: string, uuid: string, podName: string, command: string[], containerName?: string): Promise<PodCommandResponse> => {
    const response = await api.post<PodCommandResponse>(`/organizations/${organizationUuid}/application_components/webapp/${uuid}/pods/${podName}/exec`, {
      command,
  ...
  getSecrets: async (organizationUuid: string, uuid: string): Promise<{ secrets: Array<{ key: string; value: string }> }> => {
    const response = await api.get<{ secrets: Array<{ key: string; value: string }> }>(
      `/organizations/${organizationUuid}/application_components/webapp/${uuid}/secrets`
    )
    return response.data
  },
}
