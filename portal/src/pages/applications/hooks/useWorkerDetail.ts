...
export const useWorkerDetail = (organizationUuid: string | undefined, componentUuid: string | undefined, refreshInterval: number) => {
  ...
  const { data: pods = [], isLoading: isLoadingPods } = useWorkerPods(
    organizationUuid,
    componentUuid,
    refreshInterval > 0 ? refreshInterval : false
  )

  const deletePodMutation = useDeleteWorkerPod(organizationUuid)
  const execCommandMutation = useExecWorkerPodCommand(organizationUuid)

  const { data: podLogs, isLoading: isLoadingLogs } = useWorkerPodLogs(
    organizationUuid,
    componentUuid,
    selectedPod,
    undefined,
