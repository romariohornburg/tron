...
import { useWorkerComponent } from '../../features/components'
...
function WorkerDetail() {
  ...
  const { data: component, isLoading: isLoadingComponent, isError: isErrorComponent, error: errorComponent, refetch: refetchComponent } = useWorkerComponent(selectedOrganizationUuid, componentUuid)
  ...
  const {
    ...
    deletePodError,
    resetDeletePodError,
  } = useWorkerDetail(selectedOrganizationUuid, componentUuid, refreshInterval)
  ...
  const errorMessage = isErrorInstance
    ...
    : getErrorMessage(errorComponent, 'Failed to load worker. Please try again.')
  const handleRetry = () => {
    refetchInstance()
    refetchComponent()
  }
      ...
      {!isError && (
        <>
          <div className="flex items-center justify-between">
            <PageHeader
              title={component?.name || 'Worker Details'}
          ...
          {(isLoadingInstance || isLoadingComponent) ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="mt-3 text-sm text-neutral-600">Loading worker details...</p>
              </div>
            </div>
          ) : (
            <>
